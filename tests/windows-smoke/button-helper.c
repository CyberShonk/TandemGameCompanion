...
#define MODE_CHECKBOX 6
#define MODE_RADIO 7
#define MODE_OWNERDRAW 8
#define MODE_WRONG_RUNTIME_CLASS 9
#define MODE_AMBIGUOUS_PARENT 10
#define MODE_AMBIGUOUS_CONTROL 11
...
static HWND target_button = NULL;
static HWND wrong_id_button = NULL;
static HWND other_window_button = NULL;
...
static HWND create_button(
    ...
    return create_control(parent, "Button", text, style, control_id, x, y);
}
...
static void monitor_click_counts(void) {
    if (target_click_count > 1) {
        record_violation("VIOLATION-button-target-clicked-more-than-once");
    }
    if (wrong_id_click_count != 0) {
        record_violation("VIOLATION-button-correct-class-wrong-id-clicked");
    }
    if (other_window_click_count != 0) {
        record_violation("VIOLATION-button-other-window-clicked");
    }

    if (mode == MODE_IMPOSTOR && target_click_count != 0) {
        record_violation("VIOLATION-button-other-process-clicked");
    }
    if ((mode == MODE_HIDDEN
         || mode == MODE_DISABLED
         || mode == MODE_CHECKBOX
        ...
        record_violation("VIOLATION-button-rejected-control-clicked");
    }
}

static void record_final_state(void) {
    monitor_click_counts();

    if (mode == MODE_IMPOSTOR && target_click_count == 0) {
        record_event("button-other-process-final-click-count-0");
    } else if (mode == MODE_SCOPED) {
        if (target_click_count == 1) {
            record_event("button-target-final-click-count-1");
        }
        if (wrong_id_click_count == 0) {
            record_event("button-correct-class-wrong-id-final-click-count-0");
        }
        if (other_window_click_count == 0) {
            record_event("button-other-window-final-click-count-0");
        }
    } else if (mode == MODE_DEFAULT && target_click_count == 1) {
        record_event("button-default-final-click-count-1");
    }
}
    ...
    switch (message) {
        case WM_COMMAND:
            if (HIWORD(wparam) == BN_CLICKED) {
                ...
                if (window == target_window
                    && source == target_button
                    && control_id == target_control_id) {
                    target_click_count++;
                    if (mode == MODE_SCOPED && target_click_count == 1) {
                        record_event("button-target-click-1");
                        schedule_settle_close(window);
                    } else if (mode == MODE_DEFAULT && target_click_count == 1) {
                        record_event("button-default-click-1");
                        schedule_settle_close(window);
                    } else {
                        record_violation("VIOLATION-button-target-unexpected-click");
                    }
                    return 0;
                }

                if (window == target_window
                    && source == wrong_id_button
                ...
                record_violation("VIOLATION-button-unexpected-command-notification");
                return 0;
            }
            break;
        ...
        case WM_TIMER:
            ...
            if (wparam == TIMER_SHOW_DISABLED && mode == MODE_SCOPED) {
                ...
                EnableWindow(target_button, FALSE);
                ShowWindow(target_button, SW_SHOW);
                UpdateWindow(target_button);
                if (target_click_count != 0) {
                    record_violation("VIOLATION-button-clicked-before-visible-disabled-stage");
                }
                record_event("button-target-visible-disabled-click-count-0");
            ...
            if (wparam == TIMER_ENABLE_TARGET && mode == MODE_SCOPED) {
                ...
                if (target_click_count != 0) {
                    record_violation("VIOLATION-button-clicked-before-enabled-stage");
                }
                EnableWindow(target_button, TRUE);
                record_event("button-target-visible-enabled-click-count-0");
                return 0;
            }
...
static int run_impostor(
    ...
    record_event("button-impostor-start");
    target_window = create_top_level(instance, class_name, title);
    if (target_window == NULL) {
        return 4;
    }
    target_button = create_button(
        target_window,
        "Apply",
    ...
    record_event("button-impostor-ready-click-count-0");
...
static int run_scoped(
    ...
    record_event("button-scoped-start");
    target_window = create_top_level(instance, class_name, title);
    if (target_window == NULL) {
        return 4;
    }
    ...
    HWND checkbox_decoy = create_button(
        target_window,
        "Checkbox decoy",
    ...
    HWND radio_decoy = create_button(
        target_window,
        "Radio decoy",
    ...
    HWND ownerdraw_decoy = create_button(
        target_window,
        "Owner-draw decoy",
    ...
    ShowWindow(target_window, SW_SHOW);
    UpdateWindow(target_window);
    record_event("button-selector-decoys-ready");
    record_event("button-target-hidden-click-count-0");

    other_window = create_top_level(instance, class_name, "Different Button Trainer Window");
    if (other_window == NULL) {
        return 6;
    }
    other_window_button = create_button(
        other_window,
        "Apply",
        WS_VISIBLE | BS_PUSHBUTTON,
        target_control_id,
    ...
    if (other_window_button == NULL) {
        return 7;
    }
    ShowWindow(other_window, SW_SHOW);
    UpdateWindow(other_window);
    record_event("button-other-window-ready-click-count-0");
...
static int run_single_button(
    ...
    EnableWindow(target_button, enabled);
    ShowWindow(target_window, SW_SHOW);
    UpdateWindow(target_window);
...
static int run_wrong_runtime_class(
    ...
    ShowWindow(target_window, SW_SHOW);
    UpdateWindow(target_window);
    record_event("button-wrong-runtime-class-ready");
    return start_common_timers(lifetime_ms) == 0 ? 0 : 6;
}
...
static int run_ambiguous_parent(
    ...
    if (target_button == NULL || other_window_button == NULL) {
        return 5;
    }
    ShowWindow(target_window, SW_SHOW);
    ShowWindow(other_window, SW_SHOW);
    UpdateWindow(target_window);
    UpdateWindow(other_window);
    record_event("button-ambiguous-parent-ready");
    return start_common_timers(lifetime_ms) == 0 ? 0 : 6;
}
...
static int run_ambiguous_control(
    ...
    if (target_button == NULL || wrong_id_button == NULL) {
        return 5;
    }
    ShowWindow(target_window, SW_SHOW);
    UpdateWindow(target_window);
    record_event("button-ambiguous-control-ready");
    return start_common_timers(lifetime_ms) == 0 ? 0 : 6;
}
...
int main(int argc, char **argv) {
    ...
    if (strcmp(mode_name, "impostor") == 0) {
        ...
        mode = MODE_SCOPED;
    } else if (strcmp(mode_name, "default") == 0) {
        mode = MODE_DEFAULT;
    } else if (strcmp(mode_name, "hidden") == 0) {
        mode = MODE_HIDDEN;
    } else if (strcmp(mode_name, "disabled") == 0) {
        mode = MODE_DISABLED;
    } else if (strcmp(mode_name, "checkbox") == 0) {
        mode = MODE_CHECKBOX;
    ...
    window_class.lpszClassName = "TandemButtonSmokeHelper";
    ...
    if (mode == MODE_IMPOSTOR) {
        ...
        result = run_single_button(
            ...
            BS_PUSHBUTTON,
            TRUE,
            "button-hidden-ready-click-count-0",
            lifetime_ms
        );
        ...
        result = run_single_button(
            ...
            WS_VISIBLE | BS_PUSHBUTTON,
            FALSE,
            "button-disabled-ready-click-count-0",
            lifetime_ms
        );
        ...
        result = run_single_button(
            ...
            WS_VISIBLE | BS_AUTOCHECKBOX,
            TRUE,
            "button-checkbox-ready-click-count-0",
            lifetime_ms
        );
        ...
        result = run_single_button(
            ...
            WS_VISIBLE | BS_AUTORADIOBUTTON,
            TRUE,
            "button-radio-ready-click-count-0",
            lifetime_ms
        );
        ...
        result = run_single_button(
            ...
            WS_VISIBLE | BS_OWNERDRAW,
            TRUE,
            "button-ownerdraw-ready-click-count-0",
            lifetime_ms
        );
